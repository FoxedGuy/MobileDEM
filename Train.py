...
from models import create_discriminator, create_generator_small
...
def generator_images():
    for idx  in range(len(dataset[0])):
        ...
        dem_tensor = tf.convert_to_tensor(dem, dtype=tf.float32)

        yield rgb_tensor, dem_tensor


path = '/home/damian.gradziuk/mobiledem/results'
sys.path.append(path)

# load image data
dataset = load_real_samples('Dataset/Train.npz')
with tf.device('CPU'):
    train_dataset = tf.data.Dataset.from_generator(generator_images,
...
generator = create_generator_small()
...
def train_step(input_image, target):
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        ...
        gen_loss_now, gen_gan_loss, gen_l1_loss = generator_loss(disc_generated_output, gen_output, target)
        disc_loss = discriminator_loss(disc_real_output, disc_generated_output)

    generator_gradients = gen_tape.gradient(gen_lqoss_now,
...
def fit(train_ds, test_ds, epochs):
    ...
    for epoch in range(epochs):
        ...
        save_images(generator, example_input, example_target, epoch, './results')

        time_epoch = time.time() - time_start
        times.append(time_epoch)
        template = 'Epoch {}, Time: {}, Time left: {}h'
        print(template.format(epoch + 1,
                              time_epoch,
                              (epochs - epoch - 1) * (sum(times) / len(times)) / 3600))
...
fit(train_dataset, test_dataset, 400)
